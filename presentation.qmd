...
# ... (Vaš kod za učitavanje paketa i podataka ostaje isti) ...
# --- Povezivanje s DuckDB bazom podataka ---
# ... (Vaš kod za povezivanje i upit ostaje isti) ...
# --- 2. Dominacija platformi ... ---
# ... (Vaš kod za dohvaćanje platform_overview_ultimate ostaje isti) ...

# --- Definicija palete boja (iz vašeg setup chunka) ---
platform_color_mapping <- c(
  "web"       = "#003366", # Dark Blue
  "facebook"  = "#5B9BD5", # Medium Blue
  "comment"   = "#C00000", # Accent Red
  "twitter"   = "#BF8F00", # Accent Gold
  "forum"     = "#005F56", # Dark Green
  "youtube"   = "#7F7F7F", # Medium Grey
  "reddit"    = "#A0522D", # Muted Sienna
  "instagram" = "#5D478B", # Deep Slate Purple (Add if needed)
  "tiktok"    = "#8A798C"  # Muted Mauve (Add if needed)
  # Ensure all platforms from your data are here
)
...
ggplot(
  ...
  scale_x_continuous(
    labels = scales::comma,
    expand = expansion(mult = c(0.01, 0.15)) # Expand axis to fit text labels
  ) +
  guides(fill = "none") + # Hide legend as colors are mapped to axis labels
  labs(
    title = "Ukupan broj objava po platformi",
    subtitle = "",
    x = "Broj objava",
    y = "Tip platforme" # Y-axis label
  ) +

  # --- Minimalist Theme with Transparent Background ---
  theme_minimal(base_size = 14) +
  theme(
    # --- SET BACKGROUNDS TO MATCH SLIDE ---
    plot.background = element_rect(fill = "#F5F5F5", color = NA), # Use the chosen light gray, NA removes border
    panel.background = element_rect(fill = "#F5F5F5", color = NA), # Use the chosen light gray, NA removes border
    # ------------------------------------

    # Remove grid lines for minimalism
    ...
    # Adjust axis lines if desired (e.g., slightly darker gray or white)
    axis.line = element_line(color = "grey70", linewidth = 0.5), # Or element_blank()
    ...
    # --- IMPORTANT FOR LIGHT BACKGROUNDS ---
    # Ensure text colors are dark enough to be readable
    # You might need to adjust text colors if the default is too light
