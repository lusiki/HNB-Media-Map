...
# 1. Prepare clean labels as in Solution 2
topic_trends_clean <- topic_trends_guided %>%
  mutate(topic = str_replace_all(topic, "_", " ") %>% str_to_title())

# 2. Create a data frame for the labels at the end of the lines
...
ggplot(topic_trends_clean, aes(x = year, y = share, color = topic, group = topic)) +
  ...
  geom_point(size = 3) +
  scale_y_continuous(labels = scales::percent_format()) +
  scale_x_continuous(breaks = c(2021, 2022, 2023), expand = expansion(add = c(0, 0.5))) + # Add space for labels
  
  # Add the labels directly to the plot
  geom_label_repel(
    data = labels_data,
    aes(label = topic),
    nudge_x = 0.2,
